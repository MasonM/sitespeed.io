...
class HTMLBuilder {
  constructor(context, options, renderer) {
  ...
  async render(dataCollector) {
    ...
    for (let url of Object.keys(validPages)) {
      ...
      for (const summary of pageSummaries) {
        pugs[summary.id] = this.renderer.renderTemplate(summary.id, data);
      }
      data.pugs = pugs;
      ...
      for (let runIndex of Object.keys(runPages)) {
        ...
        for (const run of pageRuns) {
          pugs[run.id] = this.renderer.renderTemplate(run.id, data);
        }
        data.pugs = pugs;
        urlPageRenders.push(
  ...
  async _renderUrlPage(url, name, locals) {
    ...
    return this.storageManager.writeHtmlForUrl(
      this.renderer.renderTemplate('url/summary/' + name, locals),
      name + '.html',
      url
    );
  }
  ...
  async _renderUrlRunPage(url, name, locals) {
    ...
    return this.storageManager.writeHtmlForUrl(
      this.renderer.renderTemplate('url/iteration/index', locals),
      name + '.html',
      url
    );
  }
  ...
  async _renderSummaryPage(name, locals) {
    ...
    return this.storageManager.writeHtml(
      this.renderer.renderTemplate(name, locals),
      name + '.html'
    );
  }
}
